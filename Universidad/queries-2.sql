...
JOIN persona ON asm.id_alumno = persona.id
JOIN asignatura ON asm.id_asignatura = asignatura.id
JOIN curso_escolar ON asm.id_curso_escolar = curso_escolar.id
...
LEFT JOIN profesor ON departamento.id = profesor.id_departamento
LEFT JOIN asignatura ON profesor.id_profesor = asignatura.id_profesor
WHERE profesor.id_profesor IS NULL OR asignatura.id IS NULL;
...
FROM departamento
LEFT JOIN profesor ON departamento.id = profesor.id_departamento
GROUP BY departamento.nombre ORDER BY departamento.nombre;
...
GROUP BY grado.nombre ORDER BY total DESC;
...
SELECT grado.nombre AS grado, asignatura.tipo AS tipo,
SUM(asignatura.creditos) AS creditos_totales FROM grado
...
GROUP BY curso_escolar.anyo_inicio ORDER BY curso_escolar.anyo_inicio;
...
COUNT(asignatura.id) AS total FROM persona
JOIN profesor ON persona.id = profesor.id_profesor
LEFT JOIN asignatura ON profesor.id_profesor = asignatura.id_profesor
